...
class ThemeDefinitionReader{
  ...
  static _prepareDefinitions(def, level, parentName) {
    if(!def) {
      return;
    }
    if(Array.isArray(def)) {
      def.forEach(item => {
        // add parent and doclink info to item
        item._parent = parentName;
        item._docLink = `r_gbc_theme_vars_${parentName}/${item.name}`;
        ThemeDefinitionReader._prepareDefinitions(item, level);
      });
      return;
    }
    def._level = level;
    if(def.type === "group") {
      ThemeDefinitionReader._prepareDefinitions(def.contents, level + 1, def.name);
      return;
    }
