...
class ThemeProducer {
  ...
  analyze(filePath, contents) {
    ...
    if (deprecatedAliases.length|| undefinedVars.length) {
      console.warn(`In file ${filePath} :`);
      for (let alias of deprecatedAliases) {
        console.warn(`  Usage of '${alias.alias}' is deprecated. Use '${alias.name}' instead.`);
      }
      console.warn(`  These theme variables are set but not defined in GBC base theme definition: `);
      console.warn(undefinedVars);
    }
  }
