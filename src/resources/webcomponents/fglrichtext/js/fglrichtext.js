...
const Richtext = Class({
  constructor: function(options) {
    ...
    document.querySelector('.ql-editor').on("keydown", function(e) {
      const keyString = window.translateKeys(e);

      if (!this._editable) {
        const allowedKeys = ["home", "end", "left", "right", "up", "down", "shift+left", "shift+right", "ctrl+c", "meta+c", "ctrl+a", "meta+a"];
   ...
   */
  getHtml: function() {
    let delta = this._editor.getContents();
    delta = this.addGlobalStyle(delta);
    delta = this._cleanDelta(delta);
    const qdc = new window.QuillDeltaToHtmlConverter(delta.ops, window.opts_ || {});
    return qdc.convert();
  },
  ...
  setHtmlContent: function(htmlData, force) {
    if (!this._editing || force) {
      ...
      // Use this to prevent Quill editor to update content on Blot mutation
      const convertedParsedData = this._editor.clipboard.convert(parsedData);
      this._editor.updateContents(convertedParsedData);
    }
  },
   ...
   * @return {Boolean} true if it has focus, false otherwise
   */
  hasFocus: function() {
  ...
  addGlobalStyle: function(delta) {
    const globalStyles = Object.keys(this._globalStyle);
