 ...
 */
var onICHostReady = function(version) { // Changing var into let/const will break webcomponent.
  try {
    // Define the list of the variables you want to use as CSS variables in your Richtext
  ...
  if (version === "standalone") {
    window.gICAPI = {
      SetFocus: function() {
        console.log("gICAPI.setFocus()");
      },
      SetData: function(dataStr) {
        console.log("gICAPI.setData(\"" + dataStr + "\")");
      },
      Action: function(actionName) {
        console.log("gICAPI.action(\"" + actionName + "\")");
      },
  ...
  gICAPI.onProperty = function(property) {
    ...
    if (jsonProperty.noteditable) {
      let notEditable = jsonProperty.noteditable;
   ...
   * Called when the VM set the data of the webcomponent
   * @param data
   */
  ...
  richtext.getEditor().on('editor-change', function(evt, oldD, newD, source) {
    if(source === "user") {
      richtext.setEditing(true); // not editing if the source is not a user
    }
    else if (!_VMFocus) {
      // quill auto set focus on editor, if editor-change event is not a user
      // we must set the focus back to GBC
      parent.focus();
    }
